...
// Generated by the production Java durable loader + wire serializer from an actual native 6.24 config.
const nativeMigration = JSON.parse(readFileSync(new URL("./fixtures/native-arena-migration.json", import.meta.url), "utf8"));
...
test("actual dedicated-server status/config and Java-produced chunks reach the API", { skip: !process.env.ARENA_NATIVE_WIRE_REPORT }, async () => {
  const native = JSON.parse(readFileSync(process.env.ARENA_NATIVE_WIRE_REPORT, "utf8"));
  assert.equal(native.source, "EpreuveRuntimeProbe actual ArenaService.webContent/webStatus");
  const a = app(), originals = { execute: pool.execute, query: pool.query, getConnection: pool.getConnection };
  Object.assign(pool, databaseDouble()); const headers = { authorization: "Bearer fixture-server" };
  try {
    for (const name of ["compatible", "shipped626"]) {
      const request = native[name];
      assert.equal(request.observed.arenaConfig.schemaVersion, 2);
      let response = await a.inject({ method: "POST", url: "/api/internal/arenas/sync", headers, payload: request });
      assert.equal(response.statusCode, 200, response.body);
      for (const part of native[name + "Chunks"]) {
        response = await a.inject({ method: "POST", url: "/api/internal/arenas/sync/chunk", headers, payload: part });
        assert.equal(response.statusCode, 200, response.body);
      }
      const stored = (await a.inject({ url: "/api/admin/arenas/native-wire", headers: readHeaders })).json();
      assert.deepEqual(stored.observed, request.observed.arenaConfig);
      assert.deepEqual(stored.runtime, request.observed.runtime);
      assert.equal(stored.appliedRevision, 0); assert.equal(stored.draftRevision, 1);
    }
    assert.equal("worldStatus" in native.compatible.observed.runtime, false);
    assert.equal(typeof native.shipped626.observed.runtime.worldStatus, "string");
  } finally { Object.assign(pool, originals); await a.close(); }
});
test("6.26 worldStatus regression: native migrated catalogue reaches strict API through direct and fragmented transport", async () => {
  const a = app(), originals = { execute: pool.execute, query: pool.query, getConnection: pool.getConnection };
  Object.assign(pool, databaseDouble());
  const headers = { authorization: "Bearer fixture-server" }, route = "/api/admin/arenas/native-migration";
  const send = payload => a.inject({ method: "POST", url: "/api/internal/arenas/sync", headers, payload });
  try {
    const old = { serverId: "native-migration", appliedRevision: 0, error: "", observed: { arenaConfig: nativeMigration.previous } };
    assert.equal((await send(old)).statusCode, 200);
    const full626 = { ...structuredClone(nativeMigration.wire), serverId: "native-migration", appliedRevision: 0 };
    full626.observed.runtime = nativeMigration.nativeRuntime; // Shipped 6.26 field, rejected by the former strict runtime schema.
    let response = await send(full626); assert.equal(response.statusCode, 200, response.body);
    let stored = (await a.inject({ url: route, headers: readHeaders })).json();
    assert.equal(stored.observed.schemaVersion, 2);
    assert.deepEqual(stored.observed, nativeMigration.migrated);
    assert.deepEqual(stored.content, nativeMigration.previous, "Observation must not overwrite the owner's original draft");
    assert.deepEqual(stored.runtime, nativeMigration.nativeRuntime);
    const compatible = { ...structuredClone(nativeMigration.wire), serverId: "native-migration", appliedRevision: 0 };
    assert.equal("worldStatus" in compatible.observed.runtime, false, "Updated JAR projects the historical four-field runtime");
    response = await send(compatible); assert.equal(response.statusCode, 200, response.body);
    // Force several requests with a normal bounded catalog; snapshot data remains exact.
    const registry = catalog(); registry.items = Array.from({ length: 6500 }, (_, i) => ({ id: "minecraft:fixture_" + i, label: "É".repeat(90) }));
    full626.observed.catalog = registry;
    const parts = chunkEnvelope(full626); assert.ok(parts.length > 1);
    for (const part of parts) {
      response = await a.inject({ method: "POST", url: "/api/internal/arenas/sync/chunk", headers, payload: part });
      assert.equal(response.statusCode, 200, response.body);
    }
    stored = (await a.inject({ url: route, headers: readHeaders })).json();
    assert.deepEqual(stored.observed, nativeMigration.migrated);
    assert.deepEqual(stored.runtime, nativeMigration.nativeRuntime);
    assert.equal(stored.draftRevision, 1, "Repeated sync does not invent an edited/published revision");
    assert.equal(stored.publishedRevision, 0); assert.equal(stored.appliedRevision, 0);
    assert.deepEqual(stored.content, nativeMigration.previous);
    const invalid = structuredClone(full626); invalid.observed.runtime.worldStatus = "x".repeat(501);
    assert.equal((await send(invalid)).statusCode, 400, "Diagnostic text stays bounded");
    invalid.observed.runtime.worldStatus = 4; assert.equal((await send(invalid)).statusCode, 400);
    assert.deepEqual((await a.inject({ url: route, headers: readHeaders })).json().observed, nativeMigration.migrated, "Rejected diagnostics never roll back the good observed catalogue");
  } finally { Object.assign(pool, originals); await a.close(); }
});
...
test("schema 2 migration survives real sync, explicit draft save, publication and server acknowledgement", async () => {
  ...
  try {
    ...
    response = await a.inject({ method: "POST", url: `${route}/publish`, headers: writeHeaders, payload: { baseRevision: 1, reason: "Ancien brouillon à ne pas republier" } });
    assert.equal(response.statusCode, 409); assert.equal(response.json().error, "ARENA_SCHEMA_MIGRATION_REQUIRED", "An old-shaped edit cannot erase new Fairy/Steel authoring after migration");
