...
import { EpreuvePlanEditor } from "./EpreuvePlanEditor";
...
export function TrialEditor({ stage, content, catalog, onChange, showTrainers }: { stage: ArenaStage; content: ArenaContent; catalog: Catalog | null; onChange: (trial: TrialSpec) => void; showTrainers: () => void }) {
  ...
  if(content.schemaVersion === 2) return <EpreuvePlanEditor stage={stage} content={content} catalog={catalog} onChange={onChange} showTrainers={showTrainers}/>;
  ...
  return <section>
    ...
    {tab === "story" && <section className={s.trialStory}><div className={s.hint}><b>Un mécanisme propre à chaque épreuve.</b><p>L’énigme et le décor sont construits par le mod. Ici, tu écris l’accueil et l’indice présentés aux joueurs : aucune commande, aucun code à saisir.</p><small>Modifier un texte ne déplace pas les objets, ne change pas la solution et ne remplace pas le mécanisme de l’épreuve.</small></div><label>Présentation de l’épreuve<textarea maxLength={600} value={trial.intro} onChange={e => patch({ intro: e.target.value })}/><small>{trial.intro.length} / 600 · Ce que le joueur doit comprendre en arrivant.</small></label><label>Indice de l’énigme<textarea maxLength={400} value={trial.hint} onChange={e => patch({ hint: e.target.value })}/><small>{trial.hint.length} / 400 · Garde un indice cohérent avec le mécanisme en jeu.</small></label><div className={s.hint}><b>Le parcours n’est pas facultatif.</b><p>Dresseurs, énigme et Totem font partie des conditions du Capitaine. Masquer un dresseur le retire du parcours ; un dresseur présent reste obligatoire.</p></div></section>}
    ...
    {tab === "rewards" && <section><p className={s.hint}>Ces bonus récompensent la première victoire contre le Totem. Le badge est réservé à la victoire contre le Capitaine, et ses récompenses se règlent séparément.</p><RewardEditor rewards={trial.alpha.rewards} catalog={catalog} onChange={rewards => patch({ alpha: { ...trial.alpha, rewards } })}/></section>}
  </section>;
}
