...
export function TeamEditor({ team, catalog, level, onChange, single = false, serverId }: { team: PokemonSpec[]; catalog: Catalog | null; level: number; onChange: (team: PokemonSpec[]) => void; single?: boolean; serverId?: string }) {
  ...
  return <section className={s.teamEditor}>
    <div className={s.sectionHeading}><div><h3>{single ? "Le gardien de cette épreuve." : "Une équipe, une identité."}</h3><p>{single ? "Un Pokémon Alpha / Totem à découvrir puis à vaincre avant le Capitaine." : "De 1 à 6 Pokémon. Clique sur un membre pour préparer son combat."}</p></div><span className={s.pill}>{team.length} / {single ? 1 : 6}</span></div>
    ...
    {p && <div className={s.pokemonEditor}>
      ...
      {!single && <footer className={s.inlineActions}><button type="button" disabled={active === 0} onClick={() => { const next = [...team]; [next[active - 1], next[active]] = [next[active], next[active - 1]]; onChange(next); setSelected(active - 1); }}>← Avant</button><button type="button" disabled={active === team.length - 1} onClick={() => { const next = [...team]; [next[active + 1], next[active]] = [next[active], next[active + 1]]; onChange(next); setSelected(active + 1); }}>Après →</button><button type="button" disabled={team.length >= 6} onClick={() => { const duplicate = structuredClone(p); delete duplicate.memberId; onChange([...team, duplicate]); setSelected(team.length); }}>Dupliquer ce Pokémon</button><button type="button" className={s.danger} disabled={team.length <= 1} onClick={() => { onChange(team.filter((_, i) => i !== active)); setSelected(Math.max(0, active - 1)); }}>Retirer ce Pokémon</button></footer>}
    </div>}
  </section>;
}
