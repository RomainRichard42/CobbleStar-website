...
export default function WikiStudio() {
  ...
  return <main className={styles.studio}>
    ...
    <section className={styles.workspace}>
      ...
      <section className={styles.editor}>{article ? <><div className={styles.articleFields}><label>TITRE<input value={article.title} onChange={(event) => mutateArticle((draft) => { draft.title = event.target.value; draft.id = draft.id.startsWith("article-") ? slug(event.target.value) : draft.id; })}/></label><label>RÉSUMÉ<textarea value={article.summary} onChange={(event) => mutateArticle((draft) => { draft.summary = event.target.value; })}/></label><div><label>IMAGE<input value={article.hero.asset} onChange={(event) => mutateArticle((draft) => { draft.hero.asset = event.target.value; })}/></label><label>MOTS-CLÉS<input value={article.tags.join(", ")} onChange={(event) => mutateArticle((draft) => { draft.tags = event.target.value.split(",").map((tag) => tag.trim()).filter(Boolean); })}/></label><label className={styles.toggle}><input type="checkbox" checked={article.published} onChange={(event) => mutateArticle((draft) => { draft.published = event.target.checked; })}/> VISIBLE À LA PUBLICATION</label></div></div><div className={styles.palette}><small>AJOUTER UN BLOC</small>{palette.map(([kind, label, icon]) => <button key={kind} onClick={() => { mutateArticle((draft) => draft.blocks.push(freshBlock(kind))); setSelectedBlock(article.blocks.length); }}><span>{icon}</span>{label}</button>)}</div><div className={styles.blockList}>{article.blocks.map((item, index) => <button key={`${item.kind}-${index}`} className={index === selectedBlock ? styles.activeBlock : ""} onClick={() => setSelectedBlock(index)}><span>{String(index + 1).padStart(2, "0")}</span><b>{palette.find(([kind]) => kind === item.kind)?.[1] ?? item.kind}</b><small>{item.title ?? item.text ?? "Bloc de contenu"}</small></button>)}</div></> : <p>Aucun article dans cette branche.</p>}</section>
