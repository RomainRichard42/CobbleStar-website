...
export default function WikiAdminAccessPage() {
  ...
  return <main className={styles.page}>
    ...
    <section className={styles.content}>
      <article className={styles.intro}><small>ADMINISTRATION</small><h1>Qui peut modifier<br/><em>le wiki ?</em></h1><p>Ajoute l’adresse e-mail vérifiée du profil Discord utilisé sur CobbleStar. La personne pourra gérer les brouillons, publier les articles et ajouter d’autres administrateurs.</p><aside><b>PROTECTION DU COMPTE PRINCIPAL</b><p>L’accès initial de Romain est permanent et ne peut pas être supprimé depuis cette page.</p></aside></article>
      <article className={styles.panel}>
        <form onSubmit={add}><label>AJOUTER UN ADMINISTRATEUR<div><input type="email" required value={email} onChange={(event) => setEmail(event.target.value)} placeholder="adresse@email.fr"/><button disabled={busy}>AJOUTER L’ACCÈS</button></div><small>L’adresse peut être ajoutée avant la première connexion Discord ; elle sera reconnue automatiquement.</small></label></form>
